...
namespace Utility
{
    ...
    public:
        ...
        public:
            ...
            // return value for logging (PSM 5/23)
            long set( Item& item ) { set( item, millis_ ); }
            long set( Item& item, long millis )
            {
                if ( millis <= 0 ) { return; }
                item_ = item;
                tp_   = scheduler_.schedule( item, millis );
                return millis;
            }
            ...
            {
                cancel();
                tp_ = scheduler_.schedule( item_, millis_ );
            }
            ...
            {   // not exact, but will suffice
                if ( millis <= 0 ) { return; }
                cancel();
                tp_ = scheduler_.schedule( item_, millis + timeleft() );
            }
    ...
    //!> utility for temporary unlocking
    ...
    public:
        ~UnLocker() { lock_.lock();}
        explicit
        UnLocker(Lock& lock) : lock_(lock) { lock_.unlock(); }
    };
